...
class FamilyLoadOptions(IFamilyLoadOptions):
    ...
    def OnFamilyFound(self, familyInUse, overwriteParameterValues):
        ...
        overwriteParameterValues.Value = is_OverwriteParameter
        return True
    ...
    if element.Category.Name in TYPES_TO_COPY[1:]:
        ...
        try:
            ...
            try:
                ...
                forms.alert('Через предварительный выбор можно обновить только семейства!\n\nВыберите семейство(а) и попробуйте еще раз.')
                sys.exit()
...
if "_AR_" in doc.Title:
    pattern = ['Шаблон АР/АС','Шаблон Отливы','Шаблон Водосток', 'Шаблон Окна/Двери', 'Шаблон КЖ', 'Шаблон КМ']
else:
    pattern = ['Шаблон КЖ', 'Шаблон КМ', 'Шаблон АР/АС', 'Шаблон Окна/Двери','Шаблон Отливы','Шаблон Водосток']

def_patt = {
            'Шаблон КЖ': 'Шаблон КЖ',
            'Шаблон КМ':'Шаблон КМ',
            'Шаблон Водосток':'Шаблон Водосток',
...
if form.values:
    ...
    if not selects_symbol_names: # Ветка без предварительного выбора
        ...
        # print(len(cattegorus_familys_toload),len(familys_to_load),len(version_family)) # -> 161, 161, 161
