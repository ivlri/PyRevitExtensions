...
if form.values:
    ...
    file_pasths = {
        ...
        'Шаблон Металл':r'\\fs\public\Холдинг\ПоревитД\ТИМ\01_Библиотека\01_Рабочие задачи\ALL_Изделия металлические',
    ...
    # ===== Список всех вложеных семейств =====
    famly_inst = FilteredElementCollector(shab_doc).OfClass(FamilyInstance).ToElements()
    param_values_inst = {
                fam: fam.LookupParameter("Семейство").AsValueString()
                for fam in famly_inst
            }

    filtered_families_inst = [
                fam for fam, value in param_values_inst.items() 
                if value and ("PS_" in value or "ADSK_" in value)
            ]
    ...
    if not selects_symbol_names:
        ...
        for i in filtered_families:
            ...
            if shab_symbol_name not in names_familys_toload and shab_symbol_name not in names_supcomps:
                ...
                cattegorus_familys_toload.append(i.FamilyCategory.Name)
                names_familys_toload.append(shab_symbol_name)

        names_to_select = defaultdict(list)
        for k, v in zip(cattegorus_familys_toload, names_familys_toload):
            if v not in names_to_select[k]:
                names_to_select[k].append(v)

        dict_name_file = defaultdict(list)
        for k, v in zip(names_familys_toload,familys_to_load):
            if v not in dict_name_file[k]:
                dict_name_file[k].append(v)
        # print(dict_name_file)

        sorted_dict = defaultdict(list)
        for k in sorted(names_to_select):
            sorted_dict[k] = sorted(names_to_select[k])
