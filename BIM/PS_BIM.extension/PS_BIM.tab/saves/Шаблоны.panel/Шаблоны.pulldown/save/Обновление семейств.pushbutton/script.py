...
if select:
    try:
        selects_symbol_names = set([i.LookupParameter("Семейство").AsValueString() for i in select])
    except AttributeError:
        forms.alert('Выбраны лишние элементы\nВыбирать нужно тjлько семейства!!')
else:
    ...
    #                                                                                                                  x.LookupParameter("Семейство").AsValueString()[3:])],
...
if selects_symbol_names:
    ...
    if not to_close:
        ...
        components = [Label('Выберете шаблон для обновления семейства:'),
                    ...
                    CheckBox('OverwriteParameter', 'Загрузить с заменой параметров?'),
