...
if form.values:
    ...
    for i in doc_famly:
        print(i)
    # ===== Поиск всех семейств в открытом документе + фильтрация 
    ...
    # # ===== Поиск всех семейств в шаблоне + фильтрация 
    ...
    #         ind_to_vers = shab_filtered_families.index(i)
    ...
    #                 #Определение версии семейства в шаблоне
    #                 try:
    #                     shab_symbol = shab_doc.GetElement(list(i.GetFamilySymbolIds())[0])
    #                     shab_vers_fam = shab_symbol.LookupParameter("ADSK_Версия семейства").AsValueString()
    #                     shab_version_out = 'v{}'.format(shab_vers_fam)
    #                     start_vers_fam.append(shab_vers_fam)
    #                     if not shab_vers_fam:
    #                         shab_vers_fam = '0.0'
    #                 except:
    #                     shab_vers_fam = '0.0'
    #                     shab_version_out = ''

    #                 # print(doc_vers_fam, shab_vers_fam)
    #                 # print(shab_fam_name)
    #                 if (float(doc_vers_fam.split('.')[0]) < float(shab_vers_fam.split('.')[0]) and doc_v_out and shab_version_out):
    #                     bim_str = '(BIM)'
    #                     version_family.append('     {0} -> {1} {2}'.format(doc_v_out, shab_version_out, bim_str))
    #                     # print('1) {}'.format(shab_fam_name))
    #                 elif (float('.'.join(doc_vers_fam.split('.')[:2])) == float('.'.join(shab_vers_fam.split('.')[:2])) and doc_v_out and shab_version_out):
    ...
    #                     version_family.append('     v... -> {}'.format(shab_version_out))
    ...
    #                 # Сбор версий семейств
    #                 try:
    #                     fm_symbol = shab_doc.GetElement(list(family_to_load.GetFamilySymbolIds())[0])
    #                     end_vers_fam = fm_symbol.LookupParameter("ADSK_Версия семейства").AsValueString()
    #                     start_vers = start_vers_fam[shab_familys_to_load.index(family_to_load)]
    #                 except:
    #                     end_vers_fam = '0.0'
    #                     start_vers = '0.0'

    #                 # if int(float(start_vers)) < int(float(end_vers_fam)):
    #                 if int(start_vers.split('.')[0]) < int(end_vers_fam.split('.')[0]):
    #                     paswor = forms.ask_for_string(
    #                         default='Введите пароль:',
    #                         prompt='Обновление {}'.format(family_to_load.Name),
