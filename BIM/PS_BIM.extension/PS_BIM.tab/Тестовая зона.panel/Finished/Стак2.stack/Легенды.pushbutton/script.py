...
# === UI Выбор шаблона
components = [
    Label('Выберите файл для загрузки легенд:'),
...
with forms.ProgressBar(title='Открытие шаблона ({value}% из {max_value}%)') as pb:
    ...
    for numb, legend in enumerate(legends):
        progress = 60 + int(40 * (float(numb) / (len(legends) - 1)))
...
with forms.ProgressBar(title='Вставка видов ({value}% из {max_value}%)') as pb:
    ...
    uploaded_famuly = []
    for numb, legend in enumerate(selected_legends):
    ...
    pb._title = 'Копирование легенд ({value}% из {max_value}%)'
