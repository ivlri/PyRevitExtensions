...
# === Настройки путей открытых файлов
files_dirs = {}
doc_name = doc.Title

for open_doc in app.Documents.ForwardIterator():
    name = open_doc.Title
    if name != doc_name and not open_doc.IsLinked:
        files_dirs[open_doc.Title.split('.r')[0]] = open_doc

ordered_template_dirs.update(files_dirs)
# === UI Выбор шаблона
components = [
    Label('Выберите файл для загрузки чертежных видов'),
    ComboBox('Select_file', ordered_template_dirs, sort=False),
    Button('Подтвердить выбор')
]
form = FlexForm('Выбор файла для загрузки', components)
...
with forms.ProgressBar(title='Открытие шаблона ({value}% из {max_value}%)') as pb:
    max_value = 100
    pb.update_progress(30,max_value)
    
    #=== Подготовка файла для извлечения видов
    if "Шаблон " not in ordered_template_dirs.keys()[ordered_template_dirs.values().index(selected_path)]:
        TO_CLOSE = False
        source_doc = selected_path
    else:
    ...
    pb._title = 'Подготовка превью ({value}% из {max_value}%)'
    pb.update_progress(60,max_value)
    ...
    if not views:
        doc_close(source_doc)
        forms.alert("В шаблоне не найдено чертежных видов.", title="Ошибка")
        raise SystemExit()

    for numb, view in enumerate(views):
        progress = 60 + int(40 * (float(numb) / (len(views) - 1)))
        pb.update_progress(progress , max_value)
...
with forms.ProgressBar(title='Вставка видов ({value}% из {max_value}%)') as pb:
    max_value = 100
    ...
    if not selected_views:
        ...
        forms.alert("В выбранном файле не найдены чертежные виды.")
        doc_close(source_doc)
        raise SystemExit()
    ...
    """
    pb._title = 'Обновление семейств ({value}% из {max_value}%)'
    pb.update_progress(0,max_value)
    app.FailuresProcessing += EventHandler[FailuresProcessingEventArgs](on_failures)

    uploaded_famuly = []
    pb_len = len(selected_views)
    step = float(50) / max(1, (pb_len))
    for numb, view in enumerate(selected_views):
        progress = int(float(numb) * step)
        pb.update_progress(progress, max_value)
    ...
    pb._title = 'Копирование видов ({value}% из {max_value}%)'
    pb.update_progress(50,max_value)
    for numb, view in enumerate(selected_views):
        progress = 50 + int(float(numb) * step)
        pb.update_progress(progress, max_value)
        ...
        with DB.Transaction(doc, "Копирование чертежного вида '{}'".format(view.Name)) as t:
            ...
            t.Commit()
    pb.update_progress(100,max_value)
    doc_close(source_doc)